...
class ExecutorService:
    ...
    async def execute_plan(self, plan: PlanResult, customer_profile: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for task in plan.tasks:
            task_name = task.task.value if hasattr(task.task, "value") else str(task.task)
            if not task_name:
                continue
